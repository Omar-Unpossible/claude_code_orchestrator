...
**v1.8.1 RELEASED** ✅ (commit: `035a062`, tag: `v1.8.1`)
...
1. **Validate Issue #2** (Deliverable Assessment) - 30 min
2. **Update Configuration Guides** - 15 min
3. **Update Production Monitoring Guide** - 10 min
4. **Create Release Notes** - 15 min
5. **Update README if needed** - 10 min
...
2. Retry triggered with increased limit (5 × 3 = 15 turns)
...
**To**:
```markdown
**Result**: ✅ **VALIDATED** (or ❌ **FAILED** with details)
...
3. **Document retry behavior**:
   ...
   - Retry attempt: Multiplies by retry_multiplier (e.g., 50 × 3 = 150)
...
4. **Add examples**:
   ...
   **Example 1: Simple TASK**
   ...
   - If exceeded: Retry with 30 × 3 = 90 turns

   **Example 2: Complex STORY**
   - Task Type: STORY
   - Calculated max_turns: 50
   - If exceeded: Retry with 50 × 3 = 150 turns
   ```

### Step 2.2: Add Migration Guide (5 min)

Add a section for users upgrading from v1.8.0 → v1.8.1:
...
**What Changed**:
- Default max_turns: 10 → 50 (5x increase)
- Max limit: 30 → 150 (5x increase)
- Retry multiplier: 2 → 3 (50% increase)
...
- See `config/config.yaml` → `max_turns.by_obra_task_type` section
```
...
- CLI commands → ProductionLogger (new path)
- REPL commands → ProductionLogger (existing path)
...
**TL;DR**:
- ✅ Max_turns limits increased 5x (10 → 50) to reduce false failures
- ✅ Deliverable assessment detects partial success when max_turns hit
- ✅ Production logging now captures all CLI commands
- ✅ Fixed critical MaxTurnsCalculator initialization bug

---

## What's Fixed

### 🔴 Issue #1: Max_Turns Configuration (CRITICAL)
...
- Retry multiplier: **2 → 3** (50% increase)
- Max safety limit: **30 → 150** (5x increase)

**Impact**: Most stories now complete without hitting max_turns limit. Retries have 3x more capacity (e.g., 50 → 150).

**User Action**: None required - all changes are backward compatible.

---

### 🔴 Issue #2: Deliverable-Based Success Assessment (CRITICAL)
...
**Example CLI Output**:
```
⚠ Task completed with warnings (SUCCESS_WITH_LIMITS)
...
### 🟡 Issue #3: Production Logging for CLI (HIGH)
...
### 🔴 Issue #4: MaxTurnsCalculator Initialization Bug (CRITICAL - BLOCKER)
...
**Checklist**:
- ✅ CHANGELOG.md has v1.8.1 section (already done)
- ✅ docs/testing/ISSUE_VALIDATION_RESULTS.md updated (Issue #2 validation results)
- ✅ docs/guides/CONFIGURATION_PROFILES_GUIDE.md updated (max_turns section)
- ✅ docs/guides/PRODUCTION_MONITORING_GUIDE.md updated (CLI logging)
- ✅ docs/releases/v1.8.1_RELEASE_NOTES.md created (user-facing notes)
...
# Edit config first: default: 50 → default: 5
./venv/bin/python3 -m src.cli task execute 9 --stream
...
- TASK → 30 turns
- STORY → 50 turns (Task #9 should use this)
- EPIC → 100 turns
